...
const LoanFunds = () => {
  ...
  const renderItem = ({ id, description, lend, returned }: Loan) => {
    const totalLend = lend?.reduce((acc, amount) => acc + amount, 0) || 0;
    const totalReturned = Math.min(
      returned?.reduce((acc, amount) => acc + amount, 0) || 0,
      totalLend,
    );
          ...
          <RowView flexWrap="nowrap">
            ...
            <View style={{ flex: 1 }}>
              ...
              <RowView justifyContent="space-between">
                ...
                <Label weight="bold">{currency(rest)}</Label>
              </RowView>
            </View>
          </RowView>
        </Style_Item>
      ...
      >
        <SafeScrollView>
          ...
          {!loans ||
            (loans.length === 0 && (
              <View>
                <Label color="textSecondary" align="center">
                  Keine Einträge vorhanden.
                </Label>
                <Label color="textSecondary" align="center">
                  Klicken Sie auf das "+", um eins hinzuzufügen.
                </Label>
              </View>
            ))}
          <EditLoanModal
