...
type EditExpenseModalProps = {
  ...
  setVisible: React.Dispatch<
    React.SetStateAction<EditExpenseModalProps["visible"]>
  >;
};
...
const EditExpenseModal = ({
  ...
  const onSubmitPaidEdit = () => {
    ...
    editPaidValue &&
      onEditChange({
        paid: [...(editExpense?.paid || []), editPaid],
      });
      ...
      <View>
        ...
        <RowView justifyContent="space-between">
          <Input
            ...
            isFullWidth
          />
          <Pressable onPress={onDeletePress} hitSlop={16}>
      ...
      {editExpense?.type === "fixed" && (
        <>
          ...
          <Label size="s" color="textSecondary">
            Bezahlt{" "}
            {editExpense.paid && editExpense.paid.length > 0
              ? `(${editExpense.paid.length})`
              : ""}
          </Label>
          <Style_FlatList
